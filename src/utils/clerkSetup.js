// Clerk Setup Helper for Doctors
// This file shows how to create doctors in Clerk with publicMetadata

/*
SETUP INSTRUCTIONS:

1. In Clerk Dashboard, create doctor accounts with these usernames/emails:
   - vaidya.gopal.menon@ayurveda.com
   - vaidya.nisha.bhat@ayurveda.com
   - vaidya.priya.nair@ayurveda.com
   - etc. (from the CSV file)

2. For each doctor account, set publicMetadata in Clerk Dashboard:
   Go to Users > Select Doctor > Metadata tab > Public metadata
   
   Example for Doctor ID 1 (Vaidya Gopal Menon):
   {
     "doctorId": "1"
   }
   
   Example for Doctor ID 2 (Vaidya Nisha Bhat):
   {
     "doctorId": "2"
   }
...
// Example function to set doctor metadata (requires Clerk Backend API)
...
1. Create a doctor account in Clerk with email: vaidya.nisha.bhat@ayurveda.com
2. Set publicMetadata: { "doctorId": "2" }
3. Login as this doctor
...
2. Patient searches symptoms and finds doctors
3. Patient clicks "Book Appointment" on a doctor
...
5. Doctor logs in and sees only their appointments filtered by doctorId
*/