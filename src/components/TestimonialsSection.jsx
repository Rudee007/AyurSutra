      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {testimonials.map((testimonial, index) => (
            <div key={index} className={`bg-gradient-to-br from-green-50 to-amber-50 p-6 sm:p-8 rounded-2xl border border-green-100 shadow-md sm:hover:shadow-xl transition-all duration-500 transform sm:hover:-translate-y-3 sm:hover:scale-105 sm:hover:rotate-1 group hover:animate-none ${index === 1 ? 'md:hover:-rotate-1' : ''} ${isVisible ? 'animate-fade-in-up sm:animate-[slideInLeft_1s_ease-out] sm:[&:nth-child(2)]:animate-[slideInRight_1s_ease-out]' : 'opacity-0 translate-y-8 sm:translate-y-0 sm:translate-x-full'}`} style={{animationDelay: isVisible ? `${index * 200}ms` : '0ms'}}>
