...
function parseCSV(csvText) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      if (headerLower.includes('name') || headerLower.includes('doctor')) {
        doctor.name = value;
      } else if (headerLower.includes('special') || headerLower.includes('field')) {
        doctor.specialization = value;
      } else if (headerLower.includes('symptom') || headerLower.includes('condition') || headerLower.includes('treat')) {
        if (value) {
          doctor.symptoms_alerted = value.split(/[;|,]/).map(s => s.trim().toLowerCase()).filter(s => s.length > 0);
        }
      } else if (headerLower.includes('desc') || headerLower.includes('about')) {
  ...
  console.log('Parsed doctors:', doctors.length); // Debug log
  return doctors;
}
...
export const appointmentService = {
  async bookAppointment(appointmentData) {
    ...
    try {
      ...
      const response = await fetch('/panchakarma_doctor.json');
      if (!response.ok) {
      ...
      const scoredDoctors = doctors.map(doctor => {
        ...
        doctor.symptoms_alerted.forEach(symptom => {
          ...
          patientSymptoms.forEach(ps => {
            ...
            else if (new RegExp(`\\b${ps}\\b`).test(symptomLower) || new RegExp(`\\b${symptomLower}\\b`).test(ps)) {
              matchScore = 0.9;
            }
